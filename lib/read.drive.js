...
const downloadFile = async ({
  ...
  const drive = google.drive({ version: "v3", auth: state.auth });
  const destination = path.join(state?.rootDir, "/downloadBox");
  folderPath = path.join(destination, folderPath);
  let fileName = path.join(folderPath, _fileName);

  // if destination is not exist, create it
